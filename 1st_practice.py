...
for _ in range(buckwheat_per_month, buckwheat_bag + 1, buckwheat_per_month):
    mounth += 1
    print(
        f'Через {mounth} месяц: {buckwheat_bag - mounth * buckwheat_per_month} кг гречки в запасе')
...
for current_debtor in range(0, debtors, step):
    print(f'Должник с номером {current_debtor}')
...
for time in range(reverse_timer, 0, -1):
    ...
    if option == 1:
        print(
            f'Ваша еда готова, можете забрать! Таймер был прерван на {time} секундах.')
        break
    elif option != 0:
        print('Неверный ввод.')
        break
...
print(f'Значения функции на отрезке с шагом {step}:')
...
for month in range(1, months_of_education + 1):
    ...
    print(f'{month}-й месяц: траты {living_expenses} рублей, не хватает {expenses} рублей.')
    living_expenses = int(living_expenses * (1 + indexation))
    money_needed += expenses

print(
    f'Сумма денег, которую необходимо получить у родителей: {money_needed} рублей.')
